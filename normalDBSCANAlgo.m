function [combinedFigure] = normalDBSCANAlgo(filepath, filename)
    ...
    % Step 1: Compute the boundary using DBSCAN
    epsilon = 5; % Radius of neighborhood (tune this for your dataset)
    minPts = 5; % Minimum points in a cluster (tune this for your dataset)
    clusterLabels = dbscan(points2D, epsilon, minPts);

    % Extract boundary points from DBSCAN clusters
    boundaryPoints = [];
    for clusterIdx = unique(clusterLabels)'
        if clusterIdx > 0 % Ignore noise points (clusterIdx = -1)
            clusterPoints = points2D(clusterLabels == clusterIdx, :);
            hullIndices = convhull(clusterPoints(:, 1), clusterPoints(:, 2));
            boundaryPoints = [boundaryPoints; clusterPoints(hullIndices, :)]; %#ok<AGROW>
        end
    end

    % Safeguard against empty boundary points
    if isempty(boundaryPoints)
        warning('No boundary points detected. Check DBSCAN parameters.');
        boundaryPoints = [NaN, NaN, NaN]; % Placeholder for invalid data
    else
        % Convert the boundary points to 3D coordinates
        boundaryX = boundaryPoints(:, 1);
        boundaryY = boundaryPoints(:, 2);
        % Remove duplicate points from x and their corresponding y and z values
        [uniqueX, uniqueIdx] = unique(x, 'stable'); % Keep the first occurrence of duplicates
        uniqueY = y(uniqueIdx);
        uniqueZ = z(uniqueIdx);
        
        % Interpolate Z values for the boundary points
        boundaryZ = interp1(uniqueX, uniqueZ, boundaryPoints(:, 1), 'nearest', 'extrap'); % Estimate Z values
        boundaryPoints = [boundaryPoints(:, 1), boundaryPoints(:, 2), boundaryZ];
    ...
    combinedFigure = figure('Name', 'LiDARShape :: DBSCAN Boundary Algorithm'); % Set figure name
    ...
    % Add the heading 'DBSCAN' at the top-left inside the figure
    annotation('textbox', [0.05, 0.95, 0.5, 0.05], 'String', sprintf('DBSCAN Boundary Algorithm Result for %s', filename), ...
