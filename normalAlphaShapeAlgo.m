function [combinedFigure] = normalAlphaShapeAlgo(filepath, filename)
    ...
    points2D = [x, y]; % Use only x and y for boundary detection

    % Step 1: Compute the alpha shape for boundary detection
    alpha = estimateOptimalAlpha(points2D); % Dynamically estimate alpha value
    shp = alphaShape(points2D, alpha); % Create the alpha shape object

    % Extract boundary points from the alpha shape
    boundaryPoints = boundaryFacets(shp); % Get boundary facets of the alpha shape

    % Safeguard against empty boundary points
    if isempty(boundaryPoints)
        warning('Boundary facets are empty. Check the alpha value or input data.');
        boundaryPoints = [NaN, NaN, NaN]; % Placeholder for invalid data
    else
        % Convert the boundary facets to X, Y, Z coordinates
        boundaryX = x(boundaryPoints(:));
        boundaryY = y(boundaryPoints(:));
        boundaryZ = z(boundaryPoints(:));
        boundaryPoints = [boundaryX, boundaryY, boundaryZ];
    end
    ...
    combinedFigure = figure('Name', 'LiDARShape :: Existing Alpha Shape Algorithm'); % Set figure name

    % Set the figure size
    set(combinedFigure, 'Position', [100, 100, 1400, 500]); % Width of 1000px and height of 600px
    ...
    % Add the heading 'Normal Alpha' at the top-left inside the figure
    annotation('textbox', [0.05, 0.95, 0.5, 0.05], 'String', sprintf('Normal Alpha-Shape Algorithm Result for %s', filename), ...
        'EdgeColor', 'none', 'FontSize', 12, 'FontWeight', 'bold', 'Color', 'k', ...
        'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');

    % Add the second annotation for the alpha value
    %annotation('textbox', [0.05, 0.90, 0.5, 0.05], ... % Slightly lower vertical position (y = 0.90)
    %'String', sprintf('Calculated alpha value: %.2f', alpha), ... % Display alpha value with 2 decimal places
    %'EdgeColor', 'none', 'FontSize', 12, 'FontWeight', 'bold', 'Color', 'b', ...
    %'HorizontalAlignment', 'left', 'VerticalAlignment', 'top');
end

function alpha = estimateOptimalAlpha(points2D)
    % Dynamically estimate an optimal alpha value for the alpha shape
    dists = pdist(points2D); % Pairwise distances
    meanDist = mean(dists); % Mean distance between points
    alpha = meanDist * 3; % Increase alpha for more relaxed boundary (slower performance)
end
